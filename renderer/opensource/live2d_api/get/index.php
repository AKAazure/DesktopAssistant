...
require '../tools/externalResources.php'
...
if (is_array($modelName)) {
    ...
    $json = json_decode(file_get_contents($modelDirPath.'/model/'.$modelName.'/index.json'), 1);
} else {
    $json = json_decode(file_get_contents($modelDirPath.'/model/'.$modelName.'/index.json'), 1);
...
foreach ($json['textures'] as $k => $texture)
	$json['textures'][$k] = $modelDirPath.'/model/' . $modelName . '/' . $texture;
